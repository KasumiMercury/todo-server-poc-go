...
func TestTaskController_GetTaskById(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			userID:        testUserID,
			taskID:        testTaskID,
			mockReturn:    task.NewTaskWithoutValidation(testTaskID, "Test Task", testUserID),
			mockError:     nil,
			expectedTask:  task.NewTaskWithoutValidation(testTaskID, "Test Task", testUserID),
			expectedError: nil,
		},
		{
			name:          "task not found",
...
func TestTaskController_GetAllTasks(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			userID: testUserID,
			mockReturn: []*task.Task{
				task.NewTaskWithoutValidation(task1ID, "Task 1", testUserID),
				task.NewTaskWithoutValidation(task2ID, "Task 2", testUserID),
			},
			mockError: nil,
			expectedTasks: []*task.Task{
				task.NewTaskWithoutValidation(task1ID, "Task 1", testUserID),
...
func TestTaskController_CreateTask(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			userID:        testUserID,
			title:         "New Task",
			mockReturn:    task.NewTaskWithoutValidation(newTaskID, "New Task", testUserID),
			mockError:     nil,
			expectedTask:  task.NewTaskWithoutValidation(newTaskID, "New Task", testUserID),
			expectedError: nil,
		},
		{
...
func TestTaskController_UpdateTask(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			taskID:        testTaskID,
			title:         "Updated Task",
			mockReturn:    task.NewTaskWithoutValidation(testTaskID, "Updated Task", testUserID),
			mockError:     nil,
			expectedTask:  task.NewTaskWithoutValidation(testTaskID, "Updated Task", testUserID),
			expectedError: nil,
		},
		{
			name:          "empty title should fail validation",
